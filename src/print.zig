...
pub fn printTree(writer: anytype, root: *const tree.Node) !void {
    var guides = [_]bool{false} ** 32;

    try writer.print("{s}\n", .{root.role});
    try printAttributes(writer, root, &guides, 0, true, true);
    try printChildren(writer, root.first_child, &guides, 0);
}

fn printChildren(writer: anytype, child: ?*const tree.Node, guides: *[32]bool, depth: usize) !void {
    var current = child;
    while (current) |node| {
        const is_last = node.next_sibling == null;
        try writeIndent(writer, guides, depth);
        try writer.print("{s}{s}\n", .{ if (is_last) "└─ " else "├─ ", node.role });
        try printAttributes(writer, node, guides, depth, is_last, false);

        guides[depth] = !is_last;
        try printChildren(writer, node.first_child, guides, depth + 1);
